...
from utils import get_grad_norm, get_parameters_norm
...
class MyEngine(Engine):
    ...
    # attach calculate metrics stats to each engine
    @staticmethod
    def attach(train_engine, validation_engine, verbose=VERBOSE_BATCH_WISE):
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            # a user can execute a custom code as an event handler. Handlers can be any function: e.g. lambda, simple function, class method etc. The first argument can be optionally engine, but not necessarily.
            @train_engine.on(Events.EPOCH_COMPLETED)
            def print_train_logs(engine):
                print(
                    "Epoch {} - |param| = {:.2e} |g_param| = {:.2e} loss = {:.4e} accuracy = {:.4f}".format(
                        engine.state.epoch,
        ...
        if verbose >= VERBOSE_EPOCH_WISE:
            ...
            def print_valid_logs(engine):
                print(
                    "Validation - loss={:.4e} accuracy={:.4f} best_loss={:.4e}".format(
                        engine.state.metrics["loss"],
                        engine.state.metrics["accuracy"],
                        engine.best_loss,
                    )
...
class Trainer:
    ...
    def train(self, model, crit, optimizer, train_loader, valid_loader):
        ...
        train_engine.add_event_handler(
            Events.EPOCH_COMPLETED, run_validation, validation_engine, valid_loader  # event  # function  # arguments
        )

        validation_engine.add_event_handler(Events.EPOCH_COMPLETED, MyEngine.check_best)  # event  # function

        validation_engine.add_event_handler(
            Events.EPOCH_COMPLETED, MyEngine.save_model, train_engine, self.config  # event  # function  # arguments
        )

        # train_engine.run will execute process_function (MyEngine.train(), with arguments passed "max_epochs" times)
